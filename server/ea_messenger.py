...
class EAMessengerServer:
        ...
        for handler, row in targets:
            ...
            try:
                if not online and bool(row.get("friend", False)):
                    # Keep the durable buddy row, but do not re-announce it as
                    # online/present after the control connection closes.
                    send("RNOT", [f"CHNG={change}", f"USER={user}"])
                    send("PGET", self._social_presence_lines({**row, "online": False}))
                    continue
    ...
    def control_social_unregister_persona(self, persona: str) -> None:
        key = self._social_key(persona)
        if not key:
            return
        notify_targets: List[Tuple[object, dict]] = []
        with self._social_lock:
            handlers = set(self._social_handlers.pop(key, set()) or set())
            if handlers:
                notify_targets = self._social_notify_targets_locked(key)
            for handler in handlers:
                self._social_handler_user.pop(handler, None)
            self._social_presence.pop(key, None)
        self._social_send_presence_notifications(notify_targets, online=False)
